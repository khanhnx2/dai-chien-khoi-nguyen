...
/** Lính địch gần nhất theo khoảng cách tuyệt đối (hoặc null). */
function nearestEnemyUnit(unit: Unit, units: Unit[]): { target: Unit; dist: number } | null {
  let best: Unit | null = null;
  let bestDist = Infinity;
  for (const other of units) {
    if (other.side === unit.side || other.isDead()) continue;
